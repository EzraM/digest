      ...
      case "insert":
        if (operation.block) {
          ...
          if (operation.afterBlockId) {
            const afterIndex = this.findBlockIndex(operation.afterBlockId);
            position = afterIndex !== -1 ? afterIndex + 1 : this.yBlocks.length;
          } else {
            position = operation.position ?? this.yBlocks.length;
          }
